 ...
 */
@Named(value = "bean_Mantenimiento_Clientes")
@SessionScoped
public class Bean_Mantenimiento_Clientes implements Serializable {
     ...
     */
    public Bean_Mantenimiento_Clientes() {
...
//--------------------------------------------------------------------------
    ...
    public void ayudaEtiqueta() {
        if (bandera == 0) {
            bandera = 1;
            this.setEtiquetaAyuda("Aca podra ver, modificar, agregar, y eliminar usuarios de la base de datos");
        } else {
            bandera = 0;
    ...
    public void modificaCliente() throws SNMPExceptions, SQLException, NamingException, ClassNotFoundException {
        if (clienteSeleccionado != null) {
            this.idAntiguoUsuario = clienteSeleccionado.getIdentificacion();
            this.setEtiquetaErr("");
            ...
            if (!validaCamposNulos_PARCIAL()) {
                ...
                if (validaCorreoElectronico()) {
                    ...
                    if (!validaTarjetaCredito()) {
                        ...
                        if (!clienteSeleccionado.getIdentificacion().equals(getIdentificacion())) {
                            if (validaExistencia_Identificacion(getIdentificacion())) {
                                this.setEtiquetaErr("La identificacion ya existe");
                            }
                        }
                        ...
                        if (!clienteSeleccionado.getCorreo().equals(getCorreoElectronico())) {
                            if (validaExistencia_CorreoElectronico(getCorreoElectronico())) {
                                this.setEtiquetaErr("Correo electronico ya existente");
                            }
                        }
                        ...
                        if (!clienteSeleccionado.getTarjetaCredito().getNumTarjeta().equals(getNumeroTarjeta())) {
                            if (validaExistencia_NumeroTarjeta(getNumeroTarjeta())) {
                                this.setEtiquetaErr("Tarjeta de credito ya existente");
                            }
                        }
            ...
            this.setEtiquetaErr("Favor seleccionar un cliente primero");
        }
    }

    public void apruebaCliente() throws SNMPExceptions, SQLException, NamingException, ClassNotFoundException {
        if (clienteSeleccionado != null) {
            this.setEtiquetaErr("");
            this.log.apruebaCliente(clienteSeleccionado);
        } else {
            this.setEtiquetaErr("Favor seleccionar un cliente primero");
        }
    }

    public void denegaCliente() throws SNMPExceptions, SQLException, NamingException, ClassNotFoundException {
        if (clienteSeleccionado != null) {
            this.setEtiquetaErr("");
            this.log.denegaCliente(clienteSeleccionado);
        } else {
            this.setEtiquetaErr("Favor seleccionar un cliente primero");
        }
    }

    public void cambiaEstadoCliente() throws SNMPExceptions, SQLException, NamingException, ClassNotFoundException {
        if (clienteSeleccionado != null) {
            this.setEtiquetaErr("");
            if (clienteSeleccionado.isActivo()) {
                this.log.deshabilitaCliente(clienteSeleccionado);

            } else {
                this.log.habilitaCliente(clienteSeleccionado);
    ...
    public void inspeccionarCliente() {
        if (clienteSeleccionado != null) {
            ...
            this.setNombre(clienteSeleccionado.getNombre());
            this.setIdentificacion(clienteSeleccionado.getIdentificacion());
            this.setTelefono(clienteSeleccionado.getTelefono());
            this.setCorreoElectronico(clienteSeleccionado.getCorreo());
            this.setContrasenia(clienteSeleccionado.getContrasenia());

            this.setNumeroTarjeta(clienteSeleccionado.getTarjetaCredito().getNumTarjeta());
            this.setNumeroCVV(clienteSeleccionado.getTarjetaCredito().getCVV());
            this.setTipoTarjeta(clienteSeleccionado.getTarjetaCredito().getTipoTarjeta().toString());
            this.setVencimientoTarjeta(clienteSeleccionado.getTarjetaCredito().getMesVencimineto() + clienteSeleccionado.getTarjetaCredito().getAnnoVencimiento());
            ...
            for (Direccion posDireccione : clienteSeleccionado.getPosDirecciones()) {
                log.agregaDireccion(posDireccione);
            }
            this.setEtiquetaDirec("Registradas: " + log.cantidadDireccionGuardadas());

            for (HORARIO item : clienteSeleccionado.getHorariosEntrega()) {
                this.horarioEntrega.add(item.toString());
            }

            if (this.listaDirecciones.isEmpty()) {
                this.listaDirecciones = (LinkedList<Direccion>) clienteSeleccionado.getPosDirecciones();
            } else {
    ...
    public void buscarCliente() throws SNMPExceptions, SQLException {
        this.banderaCargaClientes = false;
        if (!(this.getClienteBuscado().equals(""))) {
            this.setEtiquetaErr("");

            this.llenaListaCliente_Aux();
            this.listaClientes.clear();

            for (Cliente item : listaClientesAux) {
                if (item.getIdentificacion().trim().equals(this.getClienteBuscado())) {
                    if (this.listaClientes.isEmpty()) {
                        this.listaClientes.add(item);
                        System.out.println("ITEM:" + item.getIdentificacion());
                        System.out.println("INGRESADA:" + this.getClienteBuscado());
                    } else {
                        this.listaClientes.clear();
                        this.listaClientes.add(item);
                    }
            ...
            this.setEtiquetaErr("Ingrese un ID en buscar cliente");
        }
    ...
    public void registrarDireccion() throws SNMPExceptions, SQLException, NamingException, ClassNotFoundException {
        if (!(clienteSeleccionado.getPosDirecciones().size() == log.cantidadDireccionGuardadas())) {
            this.setEtiquetaErr_Direcciones("");

            if (!validaExistenciaDireccion()) {
                this.setEtiquetaErr_Direcciones("");

                this.log.registrarDireccion(clienteSeleccionado);
                this.setListaDirecciones((LinkedList<Direccion>) this.log.obtieneListaDireccion(clienteSeleccionado));
    ...
    public void modificaDireccion() throws SNMPExceptions, SQLException, NamingException, ClassNotFoundException {
        if (direccionSeleccionada != null) {
            ...
            if (!validaExistenciaDireccion()) {
                ...
                log.modificaDireccion(clienteSeleccionado, direccionCliente);
                this.setListaDirecciones((LinkedList<Direccion>) this.log.obtieneListaDireccion(clienteSeleccionado));
    ...
    public void cambiaEstadoDireccion() throws SNMPExceptions, SQLException, NamingException, ClassNotFoundException {
        if (direccionSeleccionada != null) {
            ...
            if (direccionSeleccionada.isActivo()) {
                this.log.deshabilitaDireccion(clienteSeleccionado, direccionSeleccionada);
                this.setListaDirecciones((LinkedList<Direccion>) this.log.obtieneListaDireccion(clienteSeleccionado));
            } else {
                this.log.habilitaDireccion(clienteSeleccionado, direccionSeleccionada);
                this.setListaDirecciones((LinkedList<Direccion>) this.log.obtieneListaDireccion(clienteSeleccionado));
            }
        } else {
            this.setEtiquetaErr_Direcciones("Favor seleccionar una direccion primero");
    ...
    public void modificaHorario() throws SNMPExceptions, SQLException, NamingException, ClassNotFoundException {
        if (clienteSeleccionado != null) {
            ...
            if (!validaExistenciaHorario()) {
                ...
                for (HORARIO item : this.clienteSeleccionado.getHorariosEntrega()) {
                ...
                this.log.modificaHorario(this.clienteSeleccionado, lista_V, lista_N);
